...
func Test_test_example0187(t *testing.T) {
        a := Identity{}
        err := xml.Unmarshal([]byte(test_example0187), &a)
        errcheck(t, err)
        output, err := xml.Marshal(a)
        errcheck(t, err)
        want, err := mxj.NewMapXml(stripEmptyTags([]byte(test_example0187)))
        errcheck(t, err)
        mv, err := mxj.NewMapXml(stripEmptyTags(output))
        errcheck(t, err)
        if !reflect.DeepEqual(mv, want) {
                t.Fatalf("test_example0187 not DeepEqual:\nmv  : %+v\nwant: %+v\n%s\n%s\n", mv, want, string(stripEmptyTags([]byte(test_example0187))), string(stripEmptyTags(output)))
        }
}
...
func Test_test_example0804(t *testing.T) {
        a := StudentPersonal{}
        err := xml.Unmarshal([]byte(test_example0804), &a)
        errcheck(t, err)
        output, err := xml.Marshal(a)
        errcheck(t, err)
        want, err := mxj.NewMapXml(stripEmptyTags([]byte(test_example0804)))
        errcheck(t, err)
        mv, err := mxj.NewMapXml(stripEmptyTags(output))
        errcheck(t, err)
        if !reflect.DeepEqual(mv, want) {
                t.Fatalf("test_example0804 not DeepEqual:\nmv  : %+v\nwant: %+v\n%s\n%s\n", mv, want, string(stripEmptyTags([]byte(test_example0804))), string(stripEmptyTags(output)))
        }
}
...
func Test_test_example0927(t *testing.T) {
        a := PurchaseOrder{}
        err := xml.Unmarshal([]byte(test_example0927), &a)
        errcheck(t, err)
        output, err := xml.Marshal(a)
        errcheck(t, err)
        want, err := mxj.NewMapXml(stripEmptyTags([]byte(test_example0927)))
        errcheck(t, err)
        mv, err := mxj.NewMapXml(stripEmptyTags(output))
        errcheck(t, err)
        if !reflect.DeepEqual(mv, want) {
                t.Fatalf("test_example0927 not DeepEqual:\nmv  : %+v\nwant: %+v\n%s\n%s\n", mv, want, string(stripEmptyTags([]byte(test_example0927))), string(stripEmptyTags(output)))
        }
}
